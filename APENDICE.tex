...
\noindent{}Conta-se que Potemkin sofria de graves depressões, que se repetiam mais
...
assinados: \textit{Chuvalkin, Chuvalkin, Chuvalkin}\ldots{}\looseness=-1

\asterisc

\noindent{}Com dois séculos de antecipação, essa anedota anuncia a obra de Kafka. O
...
funcionário''. Georg Lukács disse uma vez que, para construir hoje uma mesa
decente, é preciso dispor do gênio arquitetônico de um Michelangelo.
...
particularmente aplicável ao filho:}\looseness=-1

\begin{quote}
O pecado original, o velho delito
...
Mas quem é acusado desse pecado original, hereditário --- o
...
e normas não são escritas. O homem pode transgredi-las sem o saber.
...
\textls[-10]{Entre a administração e a família, Kafka vê contatos múltiplos. Na
 aldeia de Schlossberg havia para isso uma expressão eloquente.}\looseness=-1

\begin{quote}
Temos aqui uma expressão que você talvez conheça: as decisões administrativas
...
Entre essas qualidades comuns talvez a mais notável fosse a de
...
senão ir mais longe, perder-se mais ainda.\looseness=-1
\end{quote}
...
\begin{quote}
É um fenômeno notável, de certo modo científico\ldots{} Não pode
...
Essas palavras estabelecem um vínculo com aqueles
...
\begin{quote}
instalaram-se num canto do chão, sobre dois velhos
...
penumbra pareciam um grande novelo.\looseness=-1 
\end{quote}

Para eles e seus semelhantes, os
...
essa arma contra Odisseus. Mas ele, informa-nos Kafka,\looseness=-1
...
\textls[-5]{Em Kafka as sereias silenciam. Talvez porque a música e o canto são para
...
descreve assim: }\looseness=-1

\begin{quote}
existe nela algo de uma infância breve e pobre, algo
...
orelha escuta tudo o que se diz.\looseness=-1

Essa tristeza profunda foi talvez um dia compensada pelo fervoroso
desejo de ``ser índio''. 

\begin{quote}
Como seria bom ser um índio, sempre pronto, a
...
já sem o pescoço do cavalo, já sem a cabeça do cavalo.
\end{quote}

Esse desejo tem
um conteúdo muito rico. Ele revela seu segredo no momento em que se
realiza: na América. A importância de \textit{Amerika} na obra de Kafka é
...
redenção''). Na China, o homem interior é 
...
Como quer que possamos traduzir
...
\textls[-10]{Assim como essa campainha,
...
habituais e explicá-los. Na \textit{Verwandlung} (em português, ``A metamorfose''), lemos sobre}\looseness=-1
...
\textls[-10]{Porém elas também são intermináveis quando partem das histórias
...
Hagadá\footnote{Hagadá, literalmente ``narração'' em hebraico, é o nome de um conjunto de textos e comentários da tradição rabínica. É voltado à interpretação das Escrituras, e serve como veículo para explorar sentidos plurais, fabulatórios e existenciais da Torá. Diferencia-se da Halachá, que trata da lei. [\textsc{n.\,e.}]} se relaciona com a Halachá. Não são parábolas e não podem
...
projetos de construção, descritos em \textit{A muralha da China}.}\looseness=-1
...
\end{quote}
 ...
 organização com palavras que poderiam ser de Kafka.\looseness=-1
...
outros esses limites. Às vezes ele se parece com o Grande Inquisidor,\footnote{O Grande Inquisidor é a figura central da parábola homônima inserida no romance \textit{Os irmãos Karamázov}, de Fiódor Dostoiévski. Nessa célebre passagem da literatura moderna, Ivan Karamázov narra a história de um inquisidor espanhol que confronta Cristo, questionando a concessão da liberdade humana, a qual considera um fardo insuportável e obstáculo à felicidade. \textsc{{[}n.\,e.{]}}} de Dostoiévski:

\begin{quote}
Estamos, portanto, em presença de um mistério, que não
...
consciência.\looseness=-1 
\end{quote}

Nem sempre Kafka resistiu às tentações do misticismo.
...
contemporâneos na mesma moeda.\looseness=-1
...
\begin{quote}
\noindent{}--- Em que teatro trabalham os senhores?

\noindent{}--- Teatro?, perguntou um deles, pedindo conselho ao outro, com os lábios trêmulos.

\noindent{}Este reagiu como um mudo, que luta com um organismo recalcitrante.
\end{quote}
...
religião.\looseness=-1

Recordemos a aldeia ao pé do castelo, do qual \textsc{k.} recebe a confirmação
...
conseguiu suportar tal atmosfera?\looseness=-1
...
merecia um castigo mais severo, como o leitor percebe claramente.\looseness=-1
...
As reflexões póstumas de Kafka, contidas em \textit{A grande muralha da China},
 ...
 como \textsc{h.\,j.}\,Schoeps, Bernhard Rang e Groethuysen. Willy Haas pode também
...
romance, \textit{Amerika}.\looseness=-1
\end{quote}

O primeiro terço dessa interpretação constitui
hoje, a partir de Brod, patrimônio comum da exegese kafkiana. Assim, por
exemplo, escreve Bernhard Rang:

\begin{quote}
Na medida em que o castelo pode ser
...
\textls[-10]{Em suas especulações bárbaras, que de resto não são sequer compatíveis
...
que lhe cabe na tradição.\looseness=-1
...
médio e o anular de sua mão direita, de modo que \textit{a película que une os
...
justifica a expressão \textit{luteae voluptates}, de Arnobius''.\looseness=-1
...
que esse ``centro misterioso'' derive da ``religião judaica''. 
...
conserva uma memória infalível até \textit{a terceira e quarta geração}, até a
\textit{centésima geração}; o momento mais sacrossanto do ritual é o apagamento
...
\begin{quote}
Aqui a plenitude do mundo é considerada a única realidade. Todo
...
senil, depois da aposentadoria''.}\looseness=-1
...
\textit{Diário}:\looseness=-1

\begin{quote}
Para ficar tão pesado quanto possível, o que considero bom
para o sono, eu cruzava os braços e punha as mãos nos ombros, como um
soldado com sua mochila. 
\end{quote}

É claro que a ideia de estar carregado tem
...
\begin{quote}
\textls[-10]{Gostaria de ser um rei poderoso,
...
neste canto, são e salvo. É o meu desejo. }\looseness=-1
\end{quote}
...
imprevistos seja insuficiente para terminar essa viagem.}\looseness=-1
\end{quote}

\textls[-15]{O mendigo é
...
qualquer desejo e o troca pela sua realização.}\looseness=-1
...
\begin{quote}
\noindent{}Ali estão as pessoas! Imaginem, elas não dormem!

\noindent{}--- E por que não?

\noindent{}--- Porque não se cansam nunca.

\noindent{}--- E por que não?

\noindent{}--- Porque são tolos.

\noindent{}--- Então os tolos não se cansam?

\noindent{}--- Como poderiam os tolos cansar-se?
\end{quote}

Como se vê, os tolos têm afinidades com
...
tranquilo ali! Como era rara a visita dos estranhos!}\looseness=-1
\end{quote}
...
Mas talvez possamos compreender
...
inclinando profundamente o rosto sobre ele''.\looseness=-1
...
de uma interpretação.\looseness=-1
...
\chapter*{Franz Kafka e o mundo invisível\footnotemark}
\addcontentsline{toc}{chapter}{Franz Kafka e o mundo invisível, \emph{por Otto M.\,Carpeaux}}
%\markboth{Introdução}{}

\footnotetext{Texto originalmente publicado no \emph{Correio da Manhã}, em 17 de abril de 1941.}
...
nossa vida cotidiana. Esta irrupção do sobrenatural no mundo não o
...
Esses terrores e esses esplendores, Kafka os escondeu nos andrajos da vida cotidiana, pois ``quem vir descoberta a face de Deus morrerá''.
...
A leste pelo apóstolo São Paulo, a oeste pela minha\qb{}educação.\footnote{Excerto de poema de Murilo Mendes. \textsc{{[}n.\,e.{]}}}
\end{verse}
\end{quote}
...
cotidiana dos escritórios, dos cafés, das casas de família; mas esses
...
execução. O delito desconhecido está vingado.\looseness=-1
...
\textls[-5]{Em todas essas parábolas,
...
O que Kafka deseja excluir é a falsa direção das nossas atividades, no sentido
...
Deste modo, \textsc{k.} encontra-se impossibilitado de
...
cotidiana sem a vocação interior. \textsc{k.} não está contente. Não quer ser
...
\begin{quote}
Quando \textsc{k.} está à morte, chega a decisão definitiva do castelo: \textsc{k.} não tem nenhum direito de permanecer na aldeia; mas considerando-se certas circunstâncias acessórias, ser-lhe-á permitido que aí permaneça até a morte.
\end{quote}
...
destinado a escrever esse fim.\looseness=-1
...
a inexistência de traduções. Uma casa editorial de Praga promete a publicação das obras completas, a \textit{Nouvelle Revue Française} traduz alguns contos. A
...
o mundo, pela estranha transposição dos acentos, pela desvalorização dos fatos tradicionais, pela revelação de um mundo mais real atrás do mundo real dos bem-pensantes: \textit{per realia ad realiora}. Eis o lema de Anton Tchekhov, a quem Kafka deve a técnica do conto. Mas
...
Tchekhov vivem no seu tempo, no tempo do seu
...
século decorre a ação dela. A era dos deuses e a vida cotidiana dos
nossos dias se confundem. Não existe tempo, há unicamente uma data: a da
irrupção do divino no mundo, acontecimento que se repete todos os dias, todas as horas.\looseness=-1
...
Julien Green. A sua vida cotidiana é destituída de sentido, como nos
...
Pierre-Jean Jouve que precedem a catástrofe. O dia de Kafka chegou.\looseness=-1
...
\begin{quote}
Como cumprir a vontade de Deus? Teme-se
